const STYLE_INSTRUCTIONS = {
  narrative:
    "纪实叙述：第三人称，按人生事件顺序推进（非段首标年），语气温暖克制，像家人讲述的真实故事；多用具体细节，少用空洞抒情。",
  literary:
...
function buildBiographyUserPrompt({ source, material, style, length, summarized, wuxiaTone }) {
  const sourceHint = SOURCE_CONTEXT[source] || SOURCE_CONTEXT.form;
  const materialLabel = summarized ? "结构化摘要（由原始素材提炼）" : "原始素材";
  const timelineHint =
    source === "timeline"
